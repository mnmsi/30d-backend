...
@section('content')
    <div class="contentInputFrom p-3">
        ...
        <div class="inputGroup mt-5">
            <form action="{{route('dua.update-item',$item->id)}}" method="post" enctype="multipart/form-data">
                @csrf
                @method('post')
                <div class="row">
                    <div class="col-6 s-12">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp"
                                   placeholder="Enter Title" value="{{$item->title}}" name="title" required>
                        </div>
                        @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <select id="inputState" class="form-control" name="category" required>
                                ...
                                @foreach($dua as $d)
                                    <option value="{{$d->id}}" {{$d->id==$item->dua_id ? "selected" : ""}} >{{$d->title}}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('category')
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control summernote" id="exampleFormControlTextarea1 description"
                                      rows="3" placeholder="Write Content" name="content_en" required>{{$item->en_text}}</textarea>
                        </div>
                        @error('content')
                        <p class="mb-0 pb-4 text-caption text-danger">{{ $message }}</p>
                        @enderror
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control summernote" id="exampleFormControlTextarea1 description"
                                      rows="3" placeholder="Write Content" name="content_ar" required>{{$item->ar_text}}</textarea>
                        </div>
                        @error('content')
                        <p class="mb-0 pb-4 text-caption text-danger">{{ $message }}</p>
                        @enderror
